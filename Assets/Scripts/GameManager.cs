...
public class GameManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Spawns environmental hazards and recusively invokes itself to ensure spawning continues. Automatically stops running when the script has been disabled.
    /// </summary>
    public void SpawnEnvironmentObject()
    {
        int environmentObject = Random.Range(0, EnvironmentPrefabs.Count);
        int spawnLocation = Random.Range(0, SpawnPoints.Count);

        GameObject.Instantiate(EnvironmentPrefabs[environmentObject], SpawnPoints[spawnLocation].transform.position, Quaternion.identity);

        Invoke("SpawnEnvironmentObject", Random.Range(5, 15));
    }
